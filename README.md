...
- `pack.mcmeta`: リソースパック定義（Resource Pack Version 84）
- `assets/monolith/items/`: アイテム定義
- `assets/monolith/models/block/`: ブロックモデル
- `assets/monolith/models/item/`: アイテムモデル
- `assets/monolith/textures/block/`: ブロック用テクスチャ
...
Minecraftのリソースパック一覧で `Monolith` を有効化し、変更後は `F3 + T` で再読み込みします。
